...
- [x] v0.4.0 — 产品逻辑重构：个人 K 线作为首页入口 + 移动端全面适配
- [x] v0.5.0 — 外服 BattleTag 接入（OverFast API）+ 实时搜索下拉 + 示例账号
- [ ] v0.6.0 — K 线缩放/平移 + RSI 背离标注 + localStorage 历史记录
- [ ] v1.0.0 — 真实社区数据后端上线
...
通过 [OverFast API](https://github.com/TeKrop/overfast-api) 读取暴雪官方公开战绩页，**仅支持本赛季数据**：

| 能力 | 状态 | 说明 |
|------|------|------|
| 本赛季英雄胜率 / KDA | ✅ 可用 | 需账号在暴雪官网设为公开 |
| 历史赛季数据 | ❌ 不可用 | 暴雪官方不保留历史赛季 |
| 逐场对局记录 | ❌ 不可用 | 暴雪不开放此接口 |

K 线图基于**赛季统计数据模拟重建**，非真实逐场记录。

### 国服（网易大神）

> **国服战绩暂无法读取真实数据。**
>
> 网易大神所有接口均需要 App 内部鉴权 Token（`GL-Token`），外部网页无法获取。经过完整的 API 逆向分析，确认目前国服守望先锋没有公开数据接口，社区也尚未找到可行的绕过方案。

**国服模式当前仅展示演示模拟数据。** 如果你有解决思路，欢迎 [提 Issue 讨论](https://github.com/bluster01/OWmarket-kline/issues)。
